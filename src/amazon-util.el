...
(defun a/parse-date-from-apollo-deployment ()
  ...
  (when (re-search-forward "^ *\\([^:]*\\)[: [:blank:]]*\\([A-Z]?[a-z]+\\) +\\([0-9]+\\), +\\([0-9]+\\) +at +\\([0-9]+\\):\\([0-9]+\\):\\([0-9]+\\) +\\([aApP][mM]\\) +GMT\\([-+][0-9]+\\)?" nil t)
    (let* ((dstr (concat (match-string 2) " "
      ...
      (save-match-data
        (let ((hour (nth 2 partial))
              (has-pm (string-match "^[pP][mM]" (match-string 8))))
          (when (or (and (eq hour 12) (not has-pm))
                    (and (not (eq hour 12) has-pm)))
...
(defun a/parse-date-from-apollo-deployment-alt ()
  ...
  (when (re-search-forward "^ *\\([^:]*\\)[: [:blank:]]*\\([0-9]+\\)/\\([0-9]+\\)/\\([0-9]+\\), +\\([0-9]+\\):\\([0-9]+\\):\\([0-9]+\\) +\\([aApP][mM]\\)" nil t)
    (list (match-beginning 0)
          ...
          (let ((hour (string-to-int (match-string 5)))
                (has-pm (char-equal ?P (upcase (aref (match-string 8) 0)))))
            (+ hour
               (if (or (and (not (eq hour 12)) has-pm)
                       (and (eq hour 12) (not has-pm)))
                   12
                 0)))
